...
/* QavanteDataTable — tabla de datos (Capa 1, agnóstico) sobre TanStack Table.
   Features: ordenamiento por columna, mostrar/ocultar columnas, y reordenar
   columnas por drag & drop (dnd-kit). Cada columna DEBE tener `id`. La lógica de
   reorden vive en data-table-utils.ts (testeada). Premium §7: header sticky,
   tabular-nums, hover de fila. */

export interface QavanteDataTableProps<TData> {
  data: TData[];
  columns: ColumnDef<TData, unknown>[];
  className?: string;
  /** Activa el reordenamiento de columnas por DnD (default true). */
  enableReorder?: boolean;
  /** Activa el panel de mostrar/ocultar columnas (default true). */
...
export function QavanteDataTable<TData>({
  data,
  columns,
  className,
  enableReorder = true,
  enableColumnToggle = true,
}: QavanteDataTableProps<TData>) {
