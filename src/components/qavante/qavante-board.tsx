...
/* QavanteBoard — tablero tipo Kanban con drag & drop entre columnas (Capa 1,
   agnóstico). Controlado: recibe `columns` y notifica `onColumnsChange`. La
   lógica de movimiento vive en board-state.ts (testeada). Accesible por teclado
   vía el sensor de dnd-kit. */

export interface QavanteBoardProps<T extends BoardItem> {
...
export function QavanteBoard<T extends BoardItem>({
  columns,
  onColumnsChange,
  renderCard,
  className,
