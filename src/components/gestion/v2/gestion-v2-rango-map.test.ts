...
import { mapRangoResumen, rangoConfiable } from "./gestion-v2-rango-map";
...
const BD: OperationalResultBreakdown = {
  ...
  rows: [
    { kind: "section", key: "income", label: "Ingresos", by_month: ["15200000", "16800000", "18500000"], total: "50500000" },
    {
      kind: "subtotal", key: "gross_margin", label: "Margen bruto",
      by_month: ["9100000", "10100000", "11100000"], total: "30300000", pct_total: "60.0", pct_by_month: ["59.9", "60.1", "60.0"],
    },
    {
      kind: "subtotal", key: "operational_result", label: "Resultado operacional",
      by_month: ["3100000", "3900000", "4500000"], total: "11500000", pct_total: "22.8", pct_by_month: ["20.4", "23.2", "24.3"],
    },
...
describe("mapRangoResumen", () => {
  ...
  it("deriva el % si el backend no manda pct_total", () => {
    const sinPct = {
      ...
      rows: [
        BD.rows[0],
        { kind: "subtotal", key: "operational_result", label: "Resultado operacional", by_month: ["1"], total: "5050000" },
      ] as OperationalResultBreakdown["rows"],
    };
...
describe("rangoConfiable", () => {
  ...
  it("NO confiable si el resultado del período supera a los ingresos", () => {
    const malo = {
      ...
      rows: [
        BD.rows[0],
        { kind: "subtotal", key: "operational_result", label: "Resultado operacional", by_month: ["1"], total: "55000000" },
      ] as OperationalResultBreakdown["rows"],
    };
