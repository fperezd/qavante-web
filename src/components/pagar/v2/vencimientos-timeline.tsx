...
export interface VencimientosTimelineProps {
  items: Vencimiento[];
  className?: string;
  /** Cuántos mostrar de entrada; el resto queda tras "Mostrar más" (evita el muro). */
...
export function VencimientosTimeline({
  items,
  className,
  initialVisible = 25,
}: VencimientosTimelineProps) {
  ...
  return (
    <>
            ...
            <>
              ...
              <span className="min-w-0">
                <span className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[13.5px] font-semibold text-neutral-dark">
                  ...
                  {tag && (
                    <span
                      className={cn("rounded-full px-2 py-0.5 text-[10.5px] font-bold", tag.cls)}
                    >
                      {tag.label}
                    </span>
                  )}
      ...
      {items.length > initialVisible && (
        <div className="flex flex-col items-center gap-1 pt-2 text-xs text-neutral-mid">
          {hasMore && (
            <QavanteButton size="sm" variant="ghost" onClick={() => setVisible((n) => n + 25)}>
              Mostrar 25 más
            </QavanteButton>
          )}
          <p className="tabular-nums">
            Mostrando {shown.length} de {items.length} · ordenados por urgencia (lo vencido primero)
          </p>
        </div>
      )}
    </>
