...
- **Tickets / PRs:** PR de este ADR + implementación acoplados (este PR).
...
- `src/test/msw/handlers.ts` — handlers REST agrupados por dominio (`auth`, `users`), alineados al shape declarado en `docs/backend-contracts/`.
- `src/test/msw/db.ts` — estado in-memory mutable (lista de usuarios, invitaciones). Mutaciones reales: invitar agrega, suspender cambia status.
- `src/test/msw/fixtures.ts` — seed determinístico (no faker random — los tests no flakean).
- `src/test/msw/browser.ts` — `setupWorker` para activación en `npm run dev` (service worker en `public/mockServiceWorker.js`).
- `src/test/msw/node.ts` — `setupServer` para vitest/playwright.
- Activación condicional via env var `NEXT_PUBLIC_API_MOCKING=enabled` + `NODE_ENV !== 'production'`. Por defecto **off**: `npm run dev` sin la var apunta al backend real.
- Carga dinámica via `import()` para que el bundle de producción no incluya `msw` (tree-shaking).

### Patterns de URL

Los handlers usan wildcard host `*/api/users` en vez de hardcoded — interceptan tanto requests a `https://api.qavante.com/api/users` (prod URL) como a `http://localhost:3000/api/users` (dev local). Esto evita acoplar MSW al valor exacto de `NEXT_PUBLIC_API_URL`.
...
- Tests unit/integration pueden cubrir happy paths reales (invitar usuario → ver la nueva fila → suspender → status cambia), no sólo middleware redirects.
...
- **+1 dependencia** (`msw@^2.x`, peso modesto, sólo devDep — fuera del bundle de prod por dynamic import).
...
- **Service worker file** (`public/mockServiceWorker.js` generado por `npx msw init`) — un archivo más a versionear pero MSW lo regenera idempotente.
- **Activación accidental en prod**: si alguien settea `NEXT_PUBLIC_API_MOCKING=enabled` en Cloudflare Workers, el flag se activa pero el doble guard (`NODE_ENV !== 'production'`) lo previene. La carga dinámica de msw tampoco está en el bundle de prod.

### Acciones que destraba o requiere

- [x] Implementación: install `msw`, setup `src/test/msw/`, handlers para auth + users (este PR).
- [x] Documentar en `CONTRIBUTING.md` cómo activar mocks en dev (este PR).
- [ ] Playwright e2e con happy paths (invite/suspend) — PR separada por scope.
...
- `docs/backend-contracts/c0-auth-and-users.md` — shape esperado de los endpoints que mockeamos.
