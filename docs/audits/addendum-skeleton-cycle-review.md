...
## Adenda post-audit (2026-05-16)

Dos follow-ups ejecutados después de cerrar el audit, dentro del mismo
ciclo (no alteran el veredicto; lo refuerzan):

- **[#106](https://github.com/fperezd/qavante-web/pull/106) — ADR-0010**:
  formaliza la decisión "selectores dependency-free, sin librería combobox"
  (preventiva, análoga a ADR-0009). Cierra la brecha de método 🟢 (decisión
  arquitectural nueva que no tenía ADR).
- **a11y correctness** en código no mergeado: se corrigió el patrón ARIA
  inválido (`role=listbox`+`<button>` anidado → lista plana + `aria-pressed`)
  en los 2 selectores (#100) y se agregó focus management al
  `ClassificationDrawer` (foco al abrir, restaurar al cerrar — WCAG modal)
  (#102). Defecto corregido **antes de merge**, no over-iteration. Suite
  scoped verde tras los fixes.

`#100`/`#102` actualizados in-place; el stack `#100→#102` se mantuvo
consistente (merge de selectores en la rama del drawer).
